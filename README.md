...
Worktrees land at `../worktrees/<repo>/<branch>` relative to the main checkout. Branch names with slashes (`feature/foo`) preserve their structure as subdirectories. Pass a separate worktree name to use one directory below the repository's worktree root instead, such as `../worktrees/<repo>/545`. The name cannot be `.`, `..`, or contain a path separator. If the branch already has a registered worktree, Perch uses its actual path and ignores the requested name.

If the worktree name collides with `ls`, `rm`, `list`, or `remove`, put `--` before it: `perch wt -- rm feature`.
